...
```bash
npm install htsl-engine
```

```js
import { compile } from "htsl-engine";
...
```js
import { installHtslRuntime } from "htsl-engine";
...
```ts
import { parse, render, compile } from "htsl-engine";
...
```ts
import htsl_engine from "htsl-engine";        // export par défaut
// ou : import { htsl_engine } from "htsl-engine";
...
import { compile, mathCss } from "htsl-engine";
...
```ts
import { hydrate, purge, loadDependency, installHtslRuntime } from "htsl-engine";
```
...
```ts
import { fromHtml, parseHtml, toHtsl } from "htsl-engine";
...
```ts
import { registry, parse } from "htsl-engine";
