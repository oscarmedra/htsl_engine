...
(`@noah-medra/htsl-core`) et les extensions d'éditeur (`@noah-medra/htsl-codemirror`) du monorepo.
...
`npm run preview`. Le moteur et `@noah-medra/htsl-codemirror` sont liés par les workspaces
...
- **Coloration**, **autocomplétion**, **indentation** et **linter** viennent de
  `@noah-medra/htsl-codemirror` (voir son README). Tab/Maj-Tab indentent.
...
**runtime unique** du moteur (`hydrate`/`purge`, voir le README de `@noah-medra/htsl-core`) sur
...
réimplémenté ici : il vient du paquet réutilisable `@noah-medra/htsl-codemirror`.
